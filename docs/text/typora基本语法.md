...
用两个`*`包裹文字,也可以用`ctrl`+`b`

**粗体**

#### 斜体：

用一个`*`包裹文字,也可以用`ctrl`+`i`

*斜体*

#### 下划线：

用`u`标签来实现，也可以用`ctrl`+`u`

<u>下划线</u>

#### 删除线：

用两个`~`包裹文字,也可以用`ctrl`+`alt`+`5`
...
在两个`:`输入需要表情符号的英文代码,也可以使用输入法中的表情或标签`Win`+`。`
